...
contract SimulatedDataSync {
    ...
    constructor(address[] memory pools) {
        ...
        for (uint256 i = 0; i < pools.length; ++i) {
            ...
            // Get tokens A and B
            IERC20 tokenA = Pool(poolAddress).tokenA();
            IERC20 tokenB = Pool(poolAddress).tokenB();

            poolData.tokenA = address(tokenA);
            poolData.tokenB = address(tokenB);
