...
pub async fn filter_pools_by_liquidity<P, T, N>(provider: Arc<P>, pools: Vec<Pool>, threshold: U256) -> Vec<Pool> 
where 
    ...
    N: Network,
{
    ...
    let pool_chunks: Vec<Vec<Pool>> = pools.chunks(50)
        .map(|chunk| {
        chunk.to_vec()
    }).collect();

    let results = stream::iter(pool_chunks).map(|chunk| {
    ...
    let flattened: Vec<(Address, U256)> = results.iter()
        .flat_map(|(addresses, values)| addresses.iter().cloned().zip(values.iter().cloned()))
        .collect();

    let filtered_addresses: Vec<Address> = flattened.into_iter()
        .filter(|(_, value)| value > &threshold)
        .map(|(address, _)| address)
        .collect();

    let filtered_pools: Vec<Pool> = pools.into_iter()
