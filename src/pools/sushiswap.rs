...
use crate::pools::{PoolFetcher, PoolType, Pool};
...
impl SushiSwapFetcher {
    ...
    where
        ...
        N: Network
    {
        let sushiswap_data: DynSolType = DynSolType::Array(Box::new(DynSolType::Tuple(vec![
            DynSolType::Address,
            DynSolType::Address,
            DynSolType::Address,
            DynSolType::Uint(8),
        ...
        loop {
            match self.attempt_build_pools(provider.clone(), &addresses, &sushiswap_data).await {
        ...
        N: Network
    {
        ...
        let decoded_data = sushiswap_data.abi_decode_sequence(&data)?;

        let mut sushiswap_pools = Vec::new();
        ...
        if let Some(pool_data_arr) = decoded_data.as_array() {
            for pool_data_tuple in pool_data_arr {
                if let Some(pool_data) = pool_data_tuple.as_tuple() {
                    let pool = SushiSwapPool::from(pool_data);
                    if pool.is_valid() {
                        sushiswap_pools.push(pool);
                    }
                }
            }
        }

        for pool in &mut sushiswap_pools {
        ...
        Ok(sushiswap_pools.into_iter().map(Pool::SushiSwap).collect())
    }
}

impl From<&[DynSolValue]> for SushiSwapPool {
