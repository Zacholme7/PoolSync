...
use crate::onchain::MaverickDataSync;
...
impl PoolBuilder for MaverickPool {
    ...
    async fn get_raw_pool_data(
        ...
        MaverickDataSync::deploy_builder(provider, addresses.to_vec())
            .call_raw()
    ...
    fn into_typed_pool(self, pool_type: PoolType) -> Pool {
        match pool_type {
            PoolType::MaverickV1 => Pool::MaverickV1(self),
            PoolType::MaverickV2 => Pool::MaverickV2(self),
            _ => panic!("Pool type not supported for Maverick structure"),
        }
    }
}

impl From<Vec<DynSolValue>> for MaverickPool {
